...
public class UpsidedownjellyEntity extends AbstractFish implements IAnimatable {


    public UpsidedownjellyEntity(EntityType<UpsidedownjellyEntity> p_29362_, Level p_29363_) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (this.isUnderWater()){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.UpsidedownJelly.swim", true));
        } else {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.UpsidedownJelly.idle", true));
        }
    ...
    private void touch(UpsidedownjellyEntity p_29606_) {
